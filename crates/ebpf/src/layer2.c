...
inline void update_mac(const IP* ip, const u8 mac[6]) {
    u64* map_mac = bpf_map_lookup_elem(&IP_TO_MAC, ip);

    // We DO NOT add mappings in the eBPF code, otherwise we could end up adding
    // entries for addresses we never send to which just bloats memory usage for
    // no purpose
    if (!map_mac) {
        return;
    }

    u64 mac_as_u64 = (u64)mac[0] << 48 | (u64)mac[1] << 32 | (u64)mac[2] << 24 |
                     (u64)mac[3] << 16 | (u64)mac[4] << 8 | (u64)mac[5] << 0;

    __sync_lock_test_and_set(map_mac, mac_as_u64);
}
...
inline XdpAction redirect_ipv6(struct xdp_md* ctx) {
    ...
    case ICMPv6:
        ...
        // target
        if (na->solicited) {
            update_mac(&v6->src_addr, na->ll_addr);
        }
...
XdpAction handle_arp(struct xdp_md* ctx) {
    ...
    update_mac(&address, arp_hdr->sender_hardware_address);
